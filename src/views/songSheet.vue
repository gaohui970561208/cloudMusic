...
<script>
import { song, v1, v3, errors } from '@/api';
...
export default {
	...
	methods: {
		...
		async getPlaylistDetail() {
			try {
				const { data, ok } = await v3.getPlaylistDetail({
					id: this.id,
					n: 100000,
				});
				if (!ok) return;
      ...
      const { data, ok } = await this.$http.post(`v1/resource/hotcomments/A_PL_0_${this.id}`, {
        rid: this.id,
        limit: 20,
        offset: 0
      })
      this.hotComments = data.hotComments
      this.hotCount = data.total
    }, */

		// 获取歌单评论
		async getPlaylistComments() {
			try {
				const { data, ok } = await v1.getPlaylistComments(this.id, {
					rid: this.id,
					limit: this.pageSize,
