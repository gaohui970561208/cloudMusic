...
<script>
import { playlist, errors } from '@/api';
...
export default {
	...
	methods: {
		...
		async getTopPlaylist() {
			try {
				const { data, ok } = await playlist.getTopPlaylist({ cat: this.tag, limit: 1 });
				if (!ok) return;
				this.topPlayList = data.playlists[0];
			} catch (error) {
				errors(error);
			}
		},
		...
		async getPlaylist() {
			try {
				const { data, ok } = await playlist.getPlaylist({
					cat: this.tag,
					limit: this.pageSize,
					total: true,
